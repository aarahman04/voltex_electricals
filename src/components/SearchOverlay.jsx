...
export default function SearchOverlay({ open, onClose }) {
  ...
  // Portalled for the same reason as MobileNav: rendered inside <header>, the
  // header's backdrop-filter becomes the containing block and clamps these
  // fixed layers to the 64px header box. One keyed motion child so the exit
  // actually runs.
  return (
    <Portal>
      <AnimatePresence>
          ...
          >
            ...
            >
              <div className="overflow-hidden rounded-[14px] border border-seam bg-paper shadow-[0_40px_80px_-32px_rgba(19,26,36,0.4)]">
                <form
                  onSubmit={submit}
                  className="flex items-center gap-3 border-b border-seam px-5 transition-colors focus-within:border-amber"
                >
                  ...
                  <div className="thin-scroll max-h-[52dvh] overflow-y-auto overscroll-contain">
                    {results.length === 0 ? (
                      <p className="px-5 py-8 text-sm text-ink-muted">
                        ...
                        a type — “Atomberg”, “1200mm”, “panel light”.
                      </p>
                    ) : (
                      <ul>
                        {results.map((p) => (
